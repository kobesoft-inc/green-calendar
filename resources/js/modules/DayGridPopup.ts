export default class DayGridPopup {
  ...
  private buildPopup(elDay: HTMLElement) {
    ...
    const elDayBody = elDay
      .querySelector('.gc-day-body')
    ...
    ;(elPopup.querySelector('.gc-date') as HTMLElement).innerText = (
      elDay.querySelector('.gc-date') as HTMLElement
    ).innerText
  }
  ...
  private getAllDayEventKeys(elDay: HTMLElement) {
    // @ts-ignore
    return Array.from(
      elDay.querySelectorAll(
        '.gc-timed-events .gc-all-day-event-container[data-key]',
      ),
    )
  ...
  private replaceAllDayEvents(elDayBody: HTMLElement, keys: Array<any>) {
    ...
    // @ts-ignore
    Array.from(
      elDayBody.querySelectorAll('.gc-all-day-event-container'),
    ...
    keys.forEach((key) => {
      const el = this._root
        .querySelector(
          '.gc-all-day-events .gc-all-day-event-container[data-key="' +
            key +
            '"]',
        )
        .cloneNode(true) as HTMLElement
  ...
  private adjustPopup(elPopup: HTMLElement) {
    ...
    // 時間指定の予定の高さを自動にする
    const elTimedEvents = elPopup.querySelector(
      '.gc-timed-events',
    ) as HTMLElement
