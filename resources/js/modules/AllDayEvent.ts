...
export default class AllDayEvent {
   ...
   */
  protected pickAllDayEvent(
    el: HTMLElement,
    withoutPopup: boolean = false,
  ): null | HTMLElement {
    return this._root.contains(el) &&
      el.closest(
        this._containerSelector + (withoutPopup ? '' : ', .gc-day-grid-popup'),
      )
  ...
  protected setHoverAllDayEvent(key: string, hover: boolean) {
    if (key) {
      this._root
        .querySelectorAll('.gc-all-day-event-container[data-key="' + key + '"]')
        .forEach((el) => {
          if (hover) {
  ...
  protected createPreview(
    ...
    // @ts-ignore
    Array.from(
      this._root.querySelectorAll('.gc-week, .gc-all-day-section'),
    ).forEach((elWeek) => {
      const [weekStart, weekEnd] = this.getWeekPeriod(elWeek)
      if (weekStart && weekEnd) {
        const [periodStart, periodEnd] = DateUtils.overlapPeriod(
          eventStart,
          eventEnd,
          weekStart,
          weekEnd,
        ...
        if (periodStart && periodEnd) {
          const elPreview = elWeek.querySelector(
            '.gc-day[data-date="' +
              periodStart +
              '"] .gc-all-day-event-preview',
          )
          if (
            weekStart <= this._resizer.getGrabbedDate() &&
            this._resizer.getGrabbedDate() <= weekEnd
          ) {
  ...
  protected getWeekPeriod(elWeek: HTMLElement): Array<any> {
    const elDays = elWeek.querySelectorAll(
      '.gc-day:not(.gc-disabled)',
  ...
  protected removePreview() {
    // @ts-ignore
    Array.from(
      this._root.querySelectorAll('.gc-all-day-event-preview'),
    ).forEach((el: Element) =>
