...
export default class TimelineLayout {
  ...
  private init() {
    ...
    this._timeSlotTotal =
      this._perDay * (DateUtils.diffDays(this._startDate, this._endDate) + 1)
  }
  ...
  public updateLayout() {
    ...
    this._root
      .querySelectorAll('.gc-events .gc-resource')
      .forEach((el: HTMLElement) => {
        this.getResourceHeadingElement(el.dataset.resourceId).style.height =
          el.offsetHeight + 'px'
      })
    ...
    this._root
      .querySelectorAll(
        '.gc-events .gc-all-day-events .gc-all-day-event-container, .gc-events .gc-timed-events .gc-timed-event-container',
      )
      .forEach((el: HTMLElement) => {
    ...
    this._root
      .querySelectorAll(
        '.gc-all-day-event-position.invisible, .gc-timed-event-position.invisible',
      )
      .forEach((el: HTMLElement) => {
  ...
  private getResourceHeadingElement(resourceId: string): HTMLElement {
    return this._root.querySelector(
      '.gc-resources .gc-resource[data-resource-id="' + resourceId + '"]',
    ) as HTMLElement
  }
  ...
  private getTimeSlotWidth(): number {
    if (this._timeSlotWidth === null) {
      this._timeSlotWidth =
        (this._root.querySelector('.gc-time-slot') as HTMLElement).offsetWidth +
        1
    }
  ...
  private getTimeSlot(index: number): HTMLElement {
    return this._root.querySelector(
      '.gc-time-slot-lines .gc-time-slot[' + 'data-index="' + index + '"]',
    )
  }
    ...
    if (this._eventHeight === null) {
      this._eventHeight = (
        this._root.querySelector(
          '.gc-events .gc-all-day-events .gc-spacer',
        ) as HTMLElement
      ).offsetHeight
    }
  ...
  public getDateTimeByIndex(index: number, isEnd: boolean = false): string {
    const d = DateUtils.addDays(
      this._startDate,
      Math.floor(index / this._perDay),
    )
    let t = this._startTime + (index % this._perDay) * this._interval
    if (this._interval === 3600) {
      // 1時間単位の場合、開始以外は1時間単位に補正する
  ...
  public getIndexByDateTime(dateTime: string, isEnd: boolean): number {
    ...
    const t = Math.floor(
      (Math.min(m, this._endTime) - this._startTime) / this._interval,
    )
  ...
  public updateEventLayout(el: HTMLElement): void {
    ...
    elPos.style.top =
      parseInt(el.dataset.position) * this.getEventHeight() + 'px'
  }
}
