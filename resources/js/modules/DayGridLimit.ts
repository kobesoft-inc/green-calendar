export default class DayGridLimit {
   ...
   */
  static readonly ANY_EVENT_SELECTOR =
    '.gc-timed-events > .gc-timed-event-container, .gc-timed-events > .gc-all-day-event-container'
   ...
   */
  public onRemainingTextClick(
    onRemainingTextClick: (elDay: HTMLElement) => void,
  ) {
  ...
  private setTimedEventsHeight(elDay: HTMLElement, height: number) {
    ;(elDay.querySelector('.gc-timed-events') as HTMLElement).style.height =
      height + 'px'
  }
  ...
  private makeRemainingText(remainingCount: number): string {
    return this._localizedRemainingText.replace(
      ':count',
