...
export default class TimedGridTimedEvent {
  ...
  private setHoverAllDayEvent(key: string, hover: boolean) {
    if (key) {
      this._root
        .querySelectorAll('.gc-timed-event-container[data-key="' + key + '"]')
        .forEach((el) => {
          if (hover) {
    ...
    Array.from(
      ...
      if (dayStart && dayEnd) {
        ...
        if (periodStart && periodEnd) {
          const [slot, span] = this.getSlotPosition(
            elDay,
            periodStart,
            periodEnd,
          )
  ...
  private getSlotPosition(
    ...
    const [dayStart, dayEnd] = this.getPeriodOfDay(elDay)
    const start = DateUtils.timeSlot(
      dayStart,
      dayEnd,
      elDay.dataset.interval,
      eventStart,
    )
    const end = DateUtils.timeSlot(
      dayStart,
      dayEnd,
      elDay.dataset.interval,
      eventEnd,
  ...
  private adjustPreview(el: HTMLElement, timeSlotHeight: number) {
    ...
    el.style.setProperty(
      '--gc-span',
      'calc(' + timeSlotHeight * 100 + '% + ' + (timeSlotHeight - 1) + 'px)',
    )
    return el
  }
  ...
  private removePreview() {
    // @ts-ignore
    Array.from(this._root.querySelectorAll('.gc-timed-event-preview')).forEach(
      (el: HTMLElement) => el.parentNode.replaceChild(el.cloneNode(false), el),
    )
  }
}
